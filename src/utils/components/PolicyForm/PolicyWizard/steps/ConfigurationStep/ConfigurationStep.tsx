...
const ConfigurationStep: FC<ConfigurationStepProps> = ({ policy, setPolicy }) => {
  ...
  const handleMTUChange = (newMTU: number) => {
    const mtuValue = newMTU > 0 ? newMTU : currentMTU;
    setPolicy((draftPolicy) => {
      draftPolicy.spec.desiredState.interfaces[0].mtu = mtuValue;
    });
    setMTUValidationMessage(validateMTU(mtuValue));
  };
    ...
    <Form className="configuration-step">
      ...
      <Stack hasGutter>
        ...
        <StackItem>
          ...
          >
            <NumberInput
              onChange={(e: ChangeEvent<HTMLInputElement>) => handleMTUChange(+e?.target?.value)}
              inputName="memory-input"
              min={1}
              onMinus={() => handleMTUChange(currentMTU - 1)}
              onPlus={() => handleMTUChange(currentMTU + 1)}
