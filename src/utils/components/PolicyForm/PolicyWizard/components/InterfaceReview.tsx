...
const InterfaceReview: FC<InterfaceReviewProps> = ({ policyInterface }) => {
  ...
  return (
    <Stack hasGutter>
      ...
      {policyInterface?.['link-aggregation']?.port && (
        <StackItem>
          <Level>
            <LevelItem>{t('Ports')}</LevelItem>
            <LevelItem>{policyInterface['link-aggregation'].port.join(', ')}</LevelItem>
          </Level>
        </StackItem>
      )}
      {policyInterface?.['link-aggregation']?.mode && (
        <StackItem>
          <Level>
            <LevelItem>{t('Aggregation mode')}</LevelItem>
            <LevelItem>{policyInterface?.['link-aggregation']?.mode}</LevelItem>
          </Level>
        </StackItem>
      )}
